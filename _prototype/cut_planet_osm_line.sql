...
CREATE TEMPORARY TABLE "insect_temp" (
    like planet_osm_line
    including all
);

insert into "insect_temp"
    select planet_osm_line.*
    from planet_osm_line, region
    where region.ref = 'RU-SPE'
    and ST_Intersects(planet_osm_line.way, region.geom)
    and not ST_Touches(planet_osm_line.way, region.geom)
    and not ST_Within(planet_osm_line.way, region.geom)
;
...
DROP TABLE IF EXISTS "RU-SPE_img"."planet_osm_line" CASCADE; 

create table "RU-SPE_img".planet_osm_line (
    like planet_osm_line
    including all
);
...
insert into "RU-SPE_img".planet_osm_line
    select 
        insect_temp.*,
        cut_temp.geom
...
insert into "RU-SPE_img".planet_osm_line
    ...
    from planet_osm_line, region
    where region.ref = 'RU-SPE'
    and ST_Within(planet_osm_line.way, region.geom)
;
