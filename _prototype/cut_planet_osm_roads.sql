...
CREATE TEMPORARY TABLE "insect_temp" (
    like planet_osm_roads
    including all
);

insert into "insect_temp"
    select planet_osm_roads.*
    from planet_osm_roads, region
    where region.ref = 'RU-SPE'
    and ST_Intersects(planet_osm_roads.way, region.geom)
    and not ST_Touches(planet_osm_roads.way, region.geom)
    and not ST_Within(planet_osm_roads.way, region.geom)
;
...
--create planet_osm_roads

DROP TABLE IF EXISTS "RU-SPE_img"."planet_osm_roads" CASCADE; 

create table "RU-SPE_img".planet_osm_roads (
    like planet_osm_roads
    including all
);
...
insert into "RU-SPE_img".planet_osm_roads
    select 
        insect_temp.*,
        cut_temp.geom
...
insert into "RU-SPE_img".planet_osm_roads
    ...
    from planet_osm_roads, region
    where region.ref = 'RU-SPE'
    and ST_Within(planet_osm_roads.way, region.geom)
;
